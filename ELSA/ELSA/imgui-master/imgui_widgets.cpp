...
//Erik leker
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, std::vector<float>* values, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
	ImGuiWindow* window = GetCurrentWindow();
	if (window->SkipItems)
		return;
	...
	if (values_count > 0)
	{
		...
		if (hovered)
		{
			...
			const float v0 = values->at((v_idx + values_offset) % values_count);
			const float v1 = values->at((v_idx + 1 + values_offset) % values_count);
		...
		float v0 = values->at((0 + values_offset) % values_count);
		float t0 = 0.0f;
		...
		for (int n = 0; n < res_w; n++)
		{
			...
			const float v1 = values->at((v1_idx + values_offset + 1) % values_count);
...
//Erik leker
//void ImGui::PlotLines(const char* label, std::vector<float>* values, void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
//{
//	PlotEx(ImGuiPlotType_Lines, label, values, data, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
//}

void ImGui::PlotLines(const char * label, std::vector<float>* values, int values_count, int values_offset, const char * overlay_text, float scale_min, float scale_max, ImVec2 graph_size, int stride)
{
	PlotEx(ImGuiPlotType_Lines, label, values, values_count, values_offset, overlay_text, scale_min, scale_max, graph_size);
}
